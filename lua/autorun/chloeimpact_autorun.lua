...
CreateClientConVar("chloeimpact_max_scale", 300, true, false, "Maximum scale for impact effects", 1, 1000)
CreateClientConVar("chloeimpact_max_debris_props", 100, true, false, "Maximum debris chunks for impact effects", 1, 1000)
CreateClientConVar("chloeimpact_max_debris_effects", 50, true, false, "Maximum dust clouds for impact effects", 1, 1000)
CreateClientConVar("chloeimpact_effects_scale", 1, true, false, "Maximum dust clouds for impact effects", 0.01, 5)
CreateClientConVar("chloeimpact_impact_lifetime", 5, true, false, "Time in seconds for impact effects to last", 1, 1000)
CreateClientConVar("chloeimpact_impact_debris_lifetime", 5, true, false, "Time in seconds for physical impact effects to last", 1, 1000)
local shouldImpact = CreateConVar("chloeimpact_enabled", 1, FCVAR_ARCHIVE, "Enable or Disable Impacts globally", 0, 1)
local VelScale = CreateConVar("chloeimpact_velocity_scale", 1, FCVAR_ARCHIVE, "Scale for velocity for impacts", 0, 15)
local DmgScale = CreateConVar("chloeimpact_damage_scale", 1, FCVAR_ARCHIVE, "Scale for damage taken from velocity", 0, 15)
local AllowPropImpact = CreateConVar("chloeimpact_prop_impact", 0, FCVAR_ARCHIVE, "(EXPERIMENTAL) Prop/Ragdoll Impact", 0, 1)
local AllowPropLodge = CreateConVar("chloeimpact_prop_lodge", 0, FCVAR_ARCHIVE, "(EXPERIMENTAL) Prop/Ragdoll Lodging", 0, 1)
local AllowPlayerLodge = CreateConVar("chloeimpact_player_lodging", 0, FCVAR_ARCHIVE, "Player Wall Lodging", 0, 1)
local gm = {}
local rocks = {
...
--resource.AddFile( "materials/models/props_debris/composite_debris.vmt" )
--resource.AddFile( "materials/models/props_debris/composite_debris2.vmt" )
--resource.AddFile( "materials/particle/particle_smoke_dust.vmt" )
--resource.AddFile( "materials/particle/particle_smokegrenade.vtf" )
--resource.AddFile( "materials/vgui/chloeimpact_overlay.vmt" )
...
hook.Add("SetupMove", "ChloeImpact_Crater", function(ply, mv, cmd) 
	if shouldImpact:GetBool() and ply:GetMoveType() ~= MOVETYPE_NOCLIP and ply:Health() > 0 then

		if DarkRP and ply.getJobTable then
			if not ply:getJobTable() then return end
			if not ply:getJobTable().impactEffects then return end
		end
...
hook.Add("PlayerSpawnedRagdoll", "ChloeImpact_RagdollCollide", function(ply, model, entity)
	if shouldImpact:GetBool() and AllowPropImpact:GetBool() then
...
function ChloeOptions(panel)
	...
	box = panel:NumSlider("Maximum debris chunks for impact effects", "chloeimpact_max_debris_props", 1, 1000, 5)
	box = panel:NumSlider("Maximum dust clouds for impact effects", "chloeimpact_max_debris_effects", 1, 1000, 5)
	box = panel:NumSlider("Scale of model effects", "chloeimpact_effects_scale", 0.01, 5, 5)
	box = panel:NumSlider("Time in seconds for physical impact effects to last", "chloeimpact_impact_debris_lifetime", 1, 1000, 5)
	box = panel:NumSlider("Time in seconds for impact effects to last", "chloeimpact_impact_lifetime", 1, 1000, 5)
	box = panel:Help("Serverside options")
	box = panel:CheckBox( "Enable Impacts", "chloeimpact_enabled") 
	box:SetValue(true)
