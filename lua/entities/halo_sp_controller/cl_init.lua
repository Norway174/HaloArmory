...
function ENT:DrawTranslucent()
    ...
    local succ, err = pcall(self.DrawControl, self)
    if not succ then
...
function ENT:DrawControl( hideButton )
    ...
    if( !network ) then
        draw.DrawText( "No Network ID", "SP_QuanticoNormal", self.frameW / 2, self.frameH / 2, self.Theme["colors"]["text_color"], TEXT_ALIGN_CENTER )
    else
        
        DrawResources( self, network )

        DrawRate( self, network )

        if !hideButton then
            DrawButton( self, network )
        end

    end

    local networkID = "0"
    if ( self.GetNetworkID and isfunction(self.GetNetworkID) ) then
        networkID = self:GetNetworkID()
    end

    // Draw ID
    draw.DrawText( "Network ID: " .. networkID, "SP_QuanticoNormal", self.frameW - 15, self.frameH - 30, Color( 173, 173, 173, 10), TEXT_ALIGN_RIGHT )
